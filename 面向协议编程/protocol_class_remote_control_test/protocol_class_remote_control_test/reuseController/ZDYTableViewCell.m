...
//    CGFloat btnHeight = self.contentView.bounds.size.height-30;
    ...
    CGFloat btnWidth = ([UIScreen mainScreen].bounds.size.width-padding*(_linenum+1))/_linenum;
    ...
    for (int i = 0; i<num; i++) {
        ...
        if (type == 0) {
            ...
            view = (UIImageView *)[reuseCache getViewWithInstanceClass:[UIImageView class] andInstanceMethod:sel andParameter:(__bridge void *)([UIImage imageNamed:@"2.jpg"])];
...
//            
        ...
        [self.contentView addSubview:view];
         //固定行高布局
         //控件大小固定,计算行高
        view.frame = CGRectMake(padding+(btnWidth+padding)*(i%_linenum), padding+(btnHeight+padding)*(i/_linenum), btnWidth, btnHeight);
    }
}

+(CGFloat)getHightWithData:(NSInteger)num{
    NSInteger row = (1+(num-1)/_linenum);
